...
ws.onmessage = function (event) {
  ...
  if (message.type === "unit_data_update") {
...
function updateUnitData(some_data) {
  const data_list = document.getElementById("unit-area");
  data_list.innerHTML = "";

  let current_class_name = null;
  let current_table = null;

  for (const unit_data of some_data) {
    if (unit_data.type !== current_class_name) {
      // New ClassName encountered, create a new table
      current_class_name = unit_data.type;
    ...
    for (const value of Object.values(unit_data)) {
