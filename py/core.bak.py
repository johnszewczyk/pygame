...
class Spawner:
    def __init__(self, gamedata) -> None:
        ...
        self.GAMESIZE = gamedata.GAMESIZE
        pass

    def makeWolf(quantity):
        """Function to spawn WOLF entity"""
        # make wolves
...
class GameMaster:
    ...
    def createEntities(self):
        ...
        # make spiders
        quantity = self.GAMESIZE // 4
        for x in range(quantity):
            new_entity = EntityData(
                icon=EntityData.EMOJI["SPIDER"],
                movetype="idle",
                index=self.randomIndex(),
                hometile=MapData.EMOJI['CABIN'],
            )
    ...
    def randomIndexType(self, entity):
        ...
        while True:
            random_index = random.randint(
                0, GameMaster.GAMESIZE * GameMaster.GAMESIZE - 1)
            if self.MapData.tilemap[random_index] == entity.hometile:
                return random_index
    ...
    def spawnPlayer(self, player):
        ...
        GameMaster.loggit(f"""{player.icon} {player.user_numb}: spawns.""")

    def spawnEntity(self, entity):
        """Function to place entity at random valid index"""

        # spawn entity to hometile type
    ...
    def moveEntityIdle(self, entity):
        ...
        if self.MapData.tilemap[entity.position] != entity.hometile:
    ...
    def movePlayer(self, player, player_input):
        ...
        GameMaster.loggit(f"""{player.icon} {player.user_numb}: move #{
                          player.move_numb}""")
        return 0
        ...
        for player in PlayerData.all_insts:
            ...
            if player_tile_type in [
                ...
                if player_tile_type == MapData.EMOJI["WATER"]:
                    GameMaster.loggit(f"""{player.icon} {
                                      player.user_numb} drowned.""")
                if player_tile_type == MapData.EMOJI["WRECKED"]:
                    GameMaster.loggit(f"""{player.icon} {
                                      player.user_numb} tripped.""")
            ...
            if player_tile_type == MapData.EMOJI["SAVED"]:
                self.newGame()
                print(TSTAMP(), "[cls.game]", player.conn, "WON")
            ...
            for entity in EntityData.all_insts:
                if player.position == entity.position:
                    ...
                    GameMaster.loggit(
                        f"""{entity.icon} > {player.icon} {player.user_numb}: #{player.loss_numb}""")
...
class AdminServer:
    ...
    async def sendEntityView(self, websocket):
        ...
        for entity in EntityData.all_insts:
            if entity.icon == EntityData.EMOJI["KNIFE"]:
...
class ClientServer:
    ...
    async def handle_connection(self, websocket, path):
        ...
        GameMaster.loggit(f"""{new_player.icon} {new_player.user_numb}: disconnected after {
            ...
            try:
                ...
                if data["type"] == "playerInput":
                    ...
                    if current_time - last_input_time >= input_interval:
                        ...
                        print(f"""{TSTAMP()} {
                              player.conn} [cls.clnt] input rate exceeded ({player_input})""")
            except websockets.ConnectionClosed:
                break
