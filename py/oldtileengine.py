...
class GameMaster:
    ...
    def randomIndexType(self, entity):
        ...
        while True:
            random_index = random.randint(
                0, self.GameData.GAMESIZE * self.GameData.GAMESIZE - 1)
            if self.MapData.tilemap[random_index] == entity.HOME_TILE:
                return random_index
    ...
    def moveEntityIdle(self, entity):
        ...
        if self.MapData.tilemap[entity.position] != entity.HOME_TILE:
        ...
        for player in PlayerData.all_insts:
            ...
            if player_tile_type in [
                ...
                if player_tile_type == MapData.EMOJI["WATER"]:
                    GameMaster.clsLog(
                        f"""{player.icon} {player.user_numb} drowned."""
                    )
                if player_tile_type == MapData.EMOJI["WRECKED"]:
                    GameMaster.clsLog(
                        f"""{player.icon} {player.user_numb} tripped."""
                    )
    ...
    def newGame(self):
        ...
        #     GAMESIZE=self.GameData.GAMESIZE,
        #     EDGESIZE=self.GameData.EDGESIZE,
        # )

        self.MapData = TileLand(self.GameData.GAMESIZE, self.GameData.EDGESIZE)
